...
from .api import AuditLogViewSet, MetadataView

audit_log_api_urls = [
    # Base URL for audit log API endpoints
    path(
        'audit/',
        include([  # Base route for audit logs
            path(
                '',  # Empty string to handle `/api/audit/` for list view
                AuditLogViewSet.as_view({'get': 'list'}),
                name='api-audit-log-list',
            ),
            path(
                'filter/',  # Filter endpoints for audit logs
                include([  # Filter specific query params
                    path(
                        'entity/',
                        AuditLogViewSet.as_view({'get': 'list'}),
                        name='api-audit-log-filter-entity',
            ...
            path(
                '<int:pk>/',  # To retrieve a specific audit log entry by its primary key
                AuditLogViewSet.as_view({'get': 'retrieve'}),
                name='api-audit-log-detail',
            ),
            path(
                '<int:pk>/metadata/',  # Metadata about the specific audit log
                MetadataView.as_view(),
                {'model': AuditLog},
                name='api-audit-log-metadata',
            ),
